...
2. `tasks/layer-N-todo.md` → Layer hiện tại, task nào đang/cần làm
...
3. Kiểm tra tất cả tasks trong layer hiện tại đã Done chưa?
   - **CÓ** → **Chạy Layer Transition:**
     a. Move completed tasks → `tasks/done.md` (section Layer N)
...
4. Nếu tất cả layers đã Done → check `tasks/layer-refinement-todo.md`

5. Đọc `docs/knowledge/INDEX.md` trước khi code — tránh lỗi cũ.
...
6. Phân tích dependency → chia layers (xem `docs/SCOPE_BREAKDOWN.md`)
7. Tạo `tasks/layer-0-todo.md`, `layer-1-todo.md`, ...
8. Chia phases phù hợp project → `docs/phases/phase-1..N.md`
9. Update file này (Stack, Folder Structure, Current Layer)
10. Hỏi user approve
...
4. `docs/decisions/` — Tại sao chọn tech stack này
5. `docs/knowledge/INDEX.md` — Lessons learned, tránh lỗi cũ
...
### Task Structure — Dependency-Driven Layers
...
Sau khi hoàn thành tất cả layers, user check lại và báo bug/feature mới.
